...
## align left 
aling_left_icon = tk.PhotoImage(file='icons/align_left.png')
aling_left_btn = ttk.Button(tool_bar, image=aling_left_icon)
aling_left_btn.grid(row=0, column=6, padx=5)

## align center
aling_center_icon = tk.PhotoImage(file='icons/align_center.png')
aling_center_btn = ttk.Button(tool_bar, image=aling_center_icon)
aling_center_btn.grid(row=0, column=7, padx=5)

## align right 
aling_right_icon = tk.PhotoImage(file='icons/align_right.png')
aling_right_btn = ttk.Button(tool_bar, image=aling_right_icon)
aling_right_btn.grid(row=0, column=8, padx=5)
...
# font familty and font size functionality
...
aling_left_btn.config(command=align_left)
...
aling_center_btn.config(command=align_center)
...
aling_right_btn.config(command=align_right)
...
def changed(event=None):
    if text_editor.edit_modified():
        ...
        charactors = len(text_editor.get(1.0, 'end-1c').replace(' ', '')) # when space is not count 
        # charactors = len(text_editor.get(1.0, 'end-1c')) # when space is count 
...
edit.add_command(label="Paste", image=past_icon,compound=tk.LEFT, accelerator="Ctrl+V", command= lambda: text_editor.event_generate("<Control v>"))
